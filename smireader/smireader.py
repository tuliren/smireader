    ...
    def __init__(self, file, encoding):
        ...
        if self.version != self.SMI_VERSION_SL4X:
            ...
            if self.parts_expected != self.parts_stored:
                logging.warning(
                    "Warning, the file has missing parts ({0} of {1} parts available)".format(
                        self.parts_stored, self.parts_expected
                    )
                )
...
class SmsSegment(object):
    ...
    def __init__(self, file, encoding='gsm'):
        ...
        if status == 1:
            self.status = self.SMS_STATUS_READ
        elif status == 3:
            self.status = self.SMS_STATUS_UNREAD
        elif status == 5:
            self.status = self.SMS_STATUS_SENT
        elif status == 7:
            self.status = self.SMS_STATUS_UNSENT
        else:
        ...
        logging.debug(
            "Segment protocol identifier: {}".format(self.protocol_identifier)
        )
        ...
        if self.segment_type == self.SMS_TYPE_SENT and self.vp_present:
            self.validity = struct.unpack(
                "B" * self.vp_length, file.read(self.vp_length)
            )
            bytes_read += self.vp_length
            logging.debug(
                "Segment validity period header read ({} bytes)".format(self.vp_length)
            )
        ...
        if encoding == 'gsm':
            ...
            for septet in zip(*[iter(pdu_bin)] * 7):
                pdu_txt.append(gsm[(int(''.join(septet), 2))])
            pdu_txt.reverse()
...
class Address(object):
    def __init__(self, file, smsc=True):
        ...
        logging.debug(
            "Reading address{} (length: {})".format(
                " of SMSC" if smsc else "", address_length
            )
        )
        if address_length:
            ...
            number = [
                "{:02X}".format(x)[::-1]
                for x in struct.unpack("B" * address_length, file.read(address_length))
            ]
...
class Timestamp(object):
    def __init__(self, file):
        ...
        logging.debug(
            "Reading raw timestamp year:{}, month:{}, day:{}, hour:{}, minute:{}, second:{}, offset:{}".format(
                year, month, day, hour, minute, second, offset
            )
        )
...
def main():
    ...
    parser.add_argument(
        "-m", "--message", help="Show message text.", action="store_true"
    )
    parser.add_argument(
        "-n",
        "--number",
    ...
    parser.add_argument(
        "-t", "--time", help="Show message date and time.", action="store_true"
    )
    parser.add_argument(
        '-v', '--verbose', default=0, help="Increase verbosity.", action='count'
    )
    parser.add_argument(
        '-e',
        '--encoding',
    ...
    parser.add_argument("file", help="File to read (*.smi and *.smo are supported)")
